get_keras_model_attn = function(hidden_layer_units = 64, embed = 1, for_card, train_dl, test_dl, output_list) {
  ...
  key = embeddings %>% (layer = layer_dense(units = 16))
  query = embeddings %>% (layer = layer_dense(units = 16))
  value = embeddings %>% (layer = layer_dense(units = 16))

  attn = layer_attention(list(key, query, value), use_scale = T)
  attn_flat = attn %>% layer_flatten() %>% layer_dropout(rate = 0.025)
