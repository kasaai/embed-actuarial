get_keras_model_tabtrans = function(hidden_layer_units = 64, embed = 0, for_card, train_dl, test_dl, output_list) {
  ...
  col_embed = layer_embedding(input_dim = 6, output_dim = 16)
  col_embeds = col_idx_int %>% time_distributed(col_embed) %>% layer_reshape(c(5,16), name = "col_embeds")
  all_embeds = list(embeddings, col_embeds) %>% layer_add()%>% layer_layer_normalization()
  ...
  stage1 = list(all_embeds, multi_head_attn) %>% layer_add()

  stage2 = stage1 %>% layer_layer_normalization() %>% layer_dense(units = 16*4, activation = "linear") %>%
    layer_activation_leaky_relu() %>%
    layer_dropout(rate = 0.05) %>% layer_dense(units = 16)

  stage3 = list(stage1, stage2) %>% layer_add()
