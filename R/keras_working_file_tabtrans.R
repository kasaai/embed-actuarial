get_keras_model_tabtrans = function(hidden_layer_units = 64, embed = 0) {
  ...
  col_embed = layer_embedding(input_dim = 6, output_dim = 8)
  col_embeds = col_idx_int %>% time_distributed(col_embed) %>% layer_reshape(c(5,8), name = "col_embeds")
  all_embeds = list(embeddings, col_embeds) %>% layer_concatenate(axis = 2)

  attn_heads = list()

  for (i in 1:4){
    key = all_embeds %>% (layer = layer_dense(units = 10))
    query = all_embeds %>% (layer = layer_dense(units = 10))
    value = all_embeds %>% (layer = layer_dense(units = 10))
    attn = layer_attention(list(key, query, value), use_scale = T)
    attn_heads[[i]] = attn
  }

  multi_head_attn = attn_heads %>% layer_concatenate(axis=2) %>% layer_dense(units = 40)
  stage1 = list(all_embeds, multi_head_attn) %>% layer_add() %>% layer_layer_normalization()

  stage2 = stage1 %>% layer_dense(units = 40*4, activation = "relu") %>% layer_dropout(rate = 0.05) %>% layer_dense(units = 40)
