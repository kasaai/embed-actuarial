...
pcas = flood_embed[[1]][1:67,] %>% princomp()
flood[, paste0("PC", 1:2) := data.table(pcas$scores[1:67,1:2])]
flood %>%
...
tsne_out <- Rtsne::Rtsne(flood_embed[[1]][1:67,], pca = FALSE, perplexity =2, eta = 100, theta = 0, max_iter = 10000)
tsne_out$Y %>%
  as.data.frame() %>%
...
p1 <- make_tsne_plot(flood_embed[[1]][1:67,], substr(flood$label, 1, 1), 2)
p2 <- make_tsne_plot(flood_embed[[1]][1:67,], substr(flood$label, 1, 1), 3)
p3 <- make_tsne_plot(flood_embed[[1]][1:67,], substr(flood$label, 1, 1), 5)
p4 <- make_tsne_plot(flood_embed[[1]][1:67,], substr(flood$label, 1, 1), 10)
library(patchwork)
...
model_attended = keras_model(inputs = model$inputs,
                             outputs = c(model$layers[[16]]$output, model$layers[[34]]$output,
...
several_pols[, paste0("flood_embed_score_",1:16) := data.table(flood_embed)]
several_pols[,  paste0("flood_embed_context_score_",1:16) := data.table(flood_embed_context)]
...
embed_dat = several_pols[, c(paste0("flood_embed_score_",1:16), "amount_paid_on_building_claim")]
cembed_dat = several_pols[, c(paste0("flood_embed_context_score_",1:16), "amount_paid_on_building_claim")]
...
data.table(`Embedding Type` = c("Embedding", "Contextual embedding"), RMSE =
  ...
  knitr::kable( digits = 0, format = "pipe")
