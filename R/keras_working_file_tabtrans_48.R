get_keras_model_tabtrans = function(hidden_layer_units = 64, embed = 0, for_card, train_dl, test_dl, output_list) {
  ...
  for (i in 1:4){
    key = all_embeds %>% (layer = layer_dense(units = 12))
    query = all_embeds %>% (layer = layer_dense(units = 12))
    value = all_embeds %>% (layer = layer_dense(units = 12))
    attn = layer_attention(list(key, query, value), use_scale = T) %>%
      layer_dropout(rate = 0.075)
    attn_heads[[i]] = attn
  }

  multi_head_attn = attn_heads %>% layer_concatenate(axis=2) %>% layer_dense(units = 48)
  stage1 = list(all_embeds, multi_head_attn) %>% layer_add()%>% layer_layer_normalization()

  stage2 = stage1  %>% layer_dense(units = 48*4, activation = "linear") %>%
    layer_activation_leaky_relu() %>%
    layer_dropout(rate = 0.15) %>% layer_dense(units = 48)

  stage3 = list(stage1, stage2) %>% layer_add() %>% layer_layer_normalization()

  attn_flat = stage3 %>% layer_flatten() %>% layer_dropout(rate = 0.15)
